...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function saveQuiz() {
    try {
      ...
      const saveResponse = await fetch("../backend/quizBackend.php", {
        ...
        body: `action=save_slides&quiz_id=${quizId}&slides=${encodeURIComponent(
  ...
  async function addCollaborator(email, permission) {
    try {
      ...
      const response = await fetch("../backend/quizBackend.php", {
        ...
        body: `action=add_collaborator&quiz_id=${quizId}&email=${encodeURIComponent(
          email
  ...
  function createNewSlide(position = -1) {
    try {
      ...
      renderSlideThumbnail(
        slideData,
        position >= 0 ? position : slides.length - 1
      );
      selectSlide(slideId);
  ...
  function renderSlideThumbnail(slideData, position) {
    const slideHTML = `
      <div class="flex items-center gap-3 mb-4 slide" data-id="${
        slideData.id
      }" draggable="true">
        <span class="w-4 text-sm text-gray-700 text-right">${
          position + 1
        }</span>
    ...
    if (position >= 0 && position < dom.slidesContainer.children.length) {
      dom.slidesContainer.children[position].insertAdjacentHTML(
        "afterend",
        slideHTML
      );
    } else {
    ...
    const newSlide = dom.slidesContainer.querySelector(
  ...
  function handleDragStart(e) {
    ...
    dragStartIndex = Array.from(dom.slidesContainer.children).indexOf(
      draggedSlide
    );
  ...
  function handleDragOver(e) {
    ...
    const insertPosition =
      offset < bounding.height / 2 ? "beforebegin" : "afterend";

    targetSlide.style.borderTop =
      insertPosition === "beforebegin" ? "2px solid #A435F0" : "none";
    targetSlide.style.borderBottom =
      insertPosition === "afterend" ? "2px solid #A435F0" : "none";
  }
  ...
  function handleDrop(e) {
    ...
    const newIndex = Array.from(dom.slidesContainer.children).indexOf(
      draggedSlide
    );
  ...
  function setupSlideEventListeners(slideElement, slideId) {
    ...
    slideElement
      .querySelector(".duplicate-slide-btn")
      ?.addEventListener("click", (e) => {
        e.stopPropagation();
    ...
    slideElement
      .querySelector(".delete-slide-btn")
      ?.addEventListener("click", (e) => {
        e.stopPropagation();
        deleteSlide(slideId);
      });
    ...
    document.querySelectorAll(".option-input").forEach((input) => {
      input.addEventListener("input", (e) => {
        ...
        const optionText = document.querySelector(
          `.option-btn[data-index="${index}"] .option-text`
        );
        if (optionText) {
    ...
    document.querySelectorAll(".delete-option-btn").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        ...
        if (currentSlide.correctAnswer >= index) {
          currentSlide.correctAnswer = Math.max(
            0,
      ...
      <div class="grid grid-cols-2 gap-6 pt-4" style="grid-auto-rows: minmax(0, 1fr); align-items: stretch;">
        ${currentSlide.options
          ...
          <div class="relative h-full">
            <button class="w-full h-full bg-${getOptionColor(
              i
            )} text-black text-xl font-medium py-4 rounded-lg hover:bg-${getOptionHoverColor(
              i
            )} transition-all option-btn ${
              currentSlide.correctAnswer === i
                ? "ring-2 ring-offset-2 ring-purple-500"
                : ""
            } min-h-[60px] whitespace-normal break-words flex items-center justify-center px-2"
                    data-index="${i}">
  ...
  function handleFileUpload() {
    ...
    if (file.type.startsWith('image/')) {
        ...
        dom.preview.innerHTML = `<p class="text-red-500 mt-2">Please upload a valid image file (JPEG, PNG, etc.).</p>`;
    }
}
  ...
  function setupEventListeners() {
    ...
    dom.openModalBtn?.addEventListener("click", () => {
      ...
      // Reset the file input when modal opens
      dom.fileInput.value = "";
    ...
    // This is the key change - make sure the file input change handler is properly set up
    dom.fileInput?.addEventListener("change", handleFileUpload);

    // Make the entire drop area clickable to trigger file selection
    dom.dropArea?.addEventListener("click", (e) => {
      // Only trigger file input if not clicking on an existing image
    ...
    dom.dropArea?.addEventListener("dragover", (e) => {
      e.preventDefault();
      dom.dropArea.classList.add("border-purple-500", "bg-purple-50");
    });

    dom.dropArea?.addEventListener("dragleave", () => {
      dom.dropArea.classList.remove("border-purple-500", "bg-purple-50");
    });
